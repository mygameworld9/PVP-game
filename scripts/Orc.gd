...
# Character Stats
@export var character_name: String = "Orc"
@export var max_health: int = 150
@export var movement_speed: float = 160.0
@export var attack_damage: int = 45
@export var attack_style: String = "brutal"
@export var attack_range: float = 85.0

# Skills
@export var skills: Array[String] = ["basic_attack", "brutal_strike", "war_cry"]
...
func _ready():
	...
	# Orc skills
	skills = ["basic_attack", "brutal_strike", "war_cry"]
	skill_cooldowns = {
		"brutal_strike": 0.0,
		"war_cry": 0.0
	}
...
func _execute_attack2():
	# Right click attack: Savage Claw
	if current_state != "attacking" and current_state != "taking_damage" and current_state != "defeated":
		print("Orc uses Attack 2: Savage Claw!")
		current_state = "attacking"
		_update_animation()
...
func _execute_special_skill():
	# Orc's special skills
	if Input.is_action_just_pressed("skill_1"):
		use_skill("brutal_strike")
	elif Input.is_action_just_pressed("skill_2"):
		use_skill("war_cry")

func _execute_brutal_strike():
	# Brutal Strike: High damage attack
	print("Orc uses Brutal Strike!")
	attack_damage = 65  # Temporary damage boost
	current_state = "attacking"
	_update_animation()
...
func _execute_war_cry():
	# War Cry: Increases damage and speed
	print("Orc uses War Cry!")
	is_enraged = true
	attack_damage = 55
	movement_speed = 180.0
	animated_sprite.modulate = Color(1.0, 0.7, 0.7, 1) # Red tint
