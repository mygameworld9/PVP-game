...
# Character Stats
@export var character_name: String = "Armored Orc"
@export var max_health: int = 180
@export var movement_speed: float = 140.0
@export var attack_damage: int = 50
@export var attack_style: String = "heavy"
@export var attack_range: float = 80.0

# Skills
@export var skills: Array[String] = ["basic_attack", "armor_break", "berserker_rage"]
...
# Armored Orc-specific properties
...
func _ready():
	...
	# Set Armored Orc-specific stats
	character_name = "Armored Orc"
	max_health = 180
	...
	# Armored Orc skills
	skills = ["basic_attack", "armor_break", "berserker_rage"]
	skill_cooldowns = {
		"armor_break": 0.0,
		"berserker_rage": 0.0
	}
...
func _execute_attack1():
	...
	if current_state != "attacking" and current_state != "taking_damage" and current_state != "defeated":
		print("Armored Orc uses Attack 1: Heavy Strike!")
...
func _execute_attack2():
	# Right click attack: Armor Crush
	if current_state != "attacking" and current_state != "taking_damage" and current_state != "defeated":
		print("Armored Orc uses Attack 2: Armor Crush!")
		current_state = "attacking"
		_update_animation()
	...
	if skill_name in skills and skill_cooldowns.get(skill_name, 0.0) <= 0.0:
		...
		match skill_name:
			"basic_attack":
				...
				if current_state == "attacking":
					...
					_update_animation()
			"armor_break":
				_execute_armor_break()
			"berserker_rage":
				_execute_berserker_rage()
		...
		if skill_name in skill_cooldowns:
			match skill_name:
				...
				"berserker_rage":
					skill_cooldowns[skill_name] = 20.0  # 20 second cooldown
		
		return true
	return false

func _execute_special_skill():
	# Armored Orc's special skills
	if Input.is_action_just_pressed("skill_1"):
		use_skill("armor_break")
	elif Input.is_action_just_pressed("skill_2"):
		use_skill("berserker_rage")

func _execute_armor_break():
	# Armor Break: Destroys enemy armor and deals massive damage
	print("Armored Orc uses Armor Break!")
	attack_damage = 80  # Temporary damage boost
	current_state = "attacking"
	_update_animation()
...
func _execute_berserker_rage():
	# Berserker Rage: Increases damage and speed, reduces health
	print("Armored Orc uses Berserker Rage!")
	is_raging = true
	attack_damage = 70
	movement_speed = 180.0
	animated_sprite.modulate = Color(1.0, 0.5, 0.5, 1) # Red tint
