...
# Character Stats
@export var character_name: String = "Orc Rider"
@export var max_health: int = 140
@export var movement_speed: float = 180.0
@export var attack_damage: int = 40
@export var attack_style: String = "mounted"
@export var attack_range: float = 90.0

# Skills
@export var skills: Array[String] = ["basic_attack", "charge_attack", "beast_command"]
...
func _ready():
	...
	# Set Orc Rider-specific stats
	character_name = "Orc Rider"
	max_health = 140
	movement_speed = 180.0
	attack_damage = 40
	attack_style = "mounted"
	attack_range = 90.0
	
	# Orc Rider skills
	skills = ["basic_attack", "charge_attack", "beast_command"]
	skill_cooldowns = {
		"charge_attack": 0.0,
		"beast_command": 0.0
	}
...
func _execute_attack2():
	# Right click attack: Beast Claw
	if current_state != "attacking" and current_state != "taking_damage" and current_state != "defeated":
		print("Orc Rider uses Attack 2: Beast Claw!")
		current_state = "attacking"
		_update_animation()
...
func _execute_special_skill():
	# Orc Rider's special skills
	if Input.is_action_just_pressed("skill_1"):
		use_skill("charge_attack")
	elif Input.is_action_just_pressed("skill_2"):
		use_skill("beast_command")

func _execute_charge_attack():
	# Charge Attack: High-speed mounted attack
	print("Orc Rider uses Charge Attack!")
	is_charging = true
...
func _execute_beast_command():
	# Beast Command: Summons beast companion
	print("Orc Rider uses Beast Command!")
	attack_damage = 50  # Temporary damage boost
	animated_sprite.modulate = Color(1.0, 0.8, 0.6, 1) # Golden tint
